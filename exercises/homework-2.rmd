...
submit_answer({
    g[c(T,F)] <- g[c(T,F)] * 3
    g[c(F,T)] <- g[c(F,T)] * 2
    g # I couldn't get it any simpler...
}, subprob = "-c")
```
...
# Bonus question: Why can't you get those 9 fish lengths by doing this: fish_len["MidOregonCoast"] ? because "MidOregonCoast" by itself is not a vector, so fish_len["MidOregonCoast"] checks only the 1st value with that name.
```
