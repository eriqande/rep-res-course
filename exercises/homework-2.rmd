...
submit_answer({
  answer_q2d <- z
  answer_q2d[is.na(answer_q2d)] <- w[is.na(answer_q2d)]
  answer_q2d
}, subprob = "-d")
...
submit_answer({
  answer_q5a <- f[c(FALSE,FALSE,TRUE)]
  answer_q5a
}, subprob = "-a")
...
# get elements 2, 5, 8, ...)
submit_answer({
  answer_q5b <- f[c(FALSE,TRUE,FALSE)]
  answer_q5b
}, subprob = "-b")
...
submit_answer({
  repunits_with_pops <- repunits
  names(repunits_with_pops) <- pops
  answer_q6a <- repunits_with_pops[fish_seq]
  answer_q6a <- unname(answer_q6a)
  answer_q6a
}, subprob = "-a")
...
submit_answer({
  answer_q6b <- fish_len
  fish_len_pops <- names(answer_q6b)
  fish_len_repunits <- repunits_with_pops[fish_len_pops]
  fish_len_repunits <- unname(fish_len_repunits)
  names(answer_q6b) <- fish_len_repunits
  fish_lr <- answer_q6b
  fish_lr
}, subprob = "-b")
...
submit_answer({
  answer_q6c <- fish_lr[names(fish_lr) == "MidOregonCoast"]
  answer_q6c <- unname(answer_q6c)
  answer_q6c
}, subprob = "-c")

# Bonus question: Why can't you get those 9 fish lengths by doing this: fish_len["MidOregonCoast"] ?

# I tried it but it only returns the first element named "MidOregonCoast" rather than all of the elements names "MidOregonCoast"
```
...
# vs decreasing order.)
submit_answer({
  answer_q7b <- len[order(wt, decreasing=TRUE)]
  answer_q7b
}, subprob = "-b")
