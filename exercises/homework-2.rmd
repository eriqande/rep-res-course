...
submit_answer({
             ...
             z[c(1,3,4,5,6,10)] #  <- put your answer to the left of the #. 
}, subprob = "-a")
...
submit_answer({f[c(FALSE,FALSE,TRUE)]
...
submit_answer({
  names(repunits) <- pops
  seqrep<-repunits[fish_seq]
  seqrep<-unname(seqrep)
  seqrep
...
# return it.
submit_answer({
  fishpops <- names(fish_len)
  fishlenrep <- repunits[fishpops]
  fishlenrep <- unname(fishlenrep)
  names(fish_len) <- fishlenrep
  fish_lr <- fish_len
  fish_lr
...
submit_answer({
  names(len)<-wt
  len[order(names(len),decreasing=TRUE)]
}, subprob = "-b")
