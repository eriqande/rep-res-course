...
submit_answer({
    ans<-z
    ans[!is.na(ans)]<-ans[!is.na(ans)]*2.5 #the second !is.na probably isn't necessary, but otherwise R was dropping a warning
    ans[is.na(ans)]<--1
    ans
}, subprob = "-c")
...
submit_answer({
    fish_lr<-fish_len
    names(fish_lr)<-repunits[names(fish_lr[pops])]
}, subprob = "-b")
