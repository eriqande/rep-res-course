...
indexing with name.  For each problem, evaluate all the code in the code chunk
...
# in the same position in the vector z
submit_answer({
...
# in the vectors where:  
#   values in "a" are between g and l, inclusive, alphabetically
# AND
#   values in "b" are less than -1.5 or greater than 1.0

# For checking, your result should have length 28.
submit_answer({
...
# here is the sequence of population-of-origin for 25 fish caught
...
# return a vector of length 25 (same as fish_seq) that gives the sequence of reporting units
...
The `sort` function 
return a sorted version of its input vector.  For example:
```{r}
r <- c(4, 7, 1, 3, 12) # not sorted

sort(r)  # returns all the elements of r in sorted order
```
This is useful when all you want to do is sort of single
...
# Recall the fish_len variable from above.  Imagine that
# the weight of each fish in kg is in the variable fish_wts.
# Note that we give it the same names attribute as fish_len.
fish_wts <- floor(rnorm(20, mean = 15, sd = 3))
names(fish_wts) <- names(fish_len)

# Problem (a): Return the vector of fish_wts sorted alphabetically
# on the population that each fish came from.
submit_answer({

}, subprob = "-a")

# Problem (b): Return fish_len sorted in DECREASING order of the
