...
submit_answer({
c[(a > "f" & a <"n") & (b < -1.5 | b > 1)]    # much less kludgy!!!         
})
```
...
submit_answer({
names(repunits) <- pops
unname(repunits[fish_seq]) #return repunits of 25 fish sampled
}, subprob = "-a")
...
# Bonus question: Why can't you get those 9 fish lengths by doing this: fish_len["MidOregonCoast"] ?  # Because the name for fish_length values
#is the population, not the reporting unit.wt
```
...
submit_answer({
names(wt) <- wpop # named wt using population names just to make sure
#that order in next step was indeed alphabetical.
wt[order(wpop)] # orders weight values based on alphabetical list of 
#populations.
}, subprob = "-a")
