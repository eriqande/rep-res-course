...
submit_answer({
  b <- c("github") # creates vector of dimension 1 with value "github"
  log_gnames <- gnames > b #creates new vector of logical values (could have simply
  # returned the vector without assighing an new name)
  log_gnames
})
...
# the word "github" alphabetically
submit_answer({
gnames[c(gnames > b)] #indexes gnames by the logical value of the comparison to "b"
})
```
...
ords <- sample(length(gnames)) #not sure what "length" means here.  

# return the first 4 github names that were signed up
submit_answer({
gnames [c(ords < 5)] #selects elements of gnames that correspond to Ord values 1:4
})
```
